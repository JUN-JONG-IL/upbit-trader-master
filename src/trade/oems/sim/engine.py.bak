...
설명:
- 패키지 경로에 숫자가 포함된 폴더명(예: 10_trade)이 있어 'from src.10_trade.oems import ...' 같은
...
# 동적 임포트: '10_trade' 같은 숫자 시작 디렉터리 때문에 정적 import가 실패할 수 있으므로
# importlib.import_module로 모듈을 로드합니다.
try:
    router = importlib.import_module("src.10_trade.oems.router")
except Exception:
    logger.exception("[engine] router 모듈 로드 실패")
    router = None  # type: ignore

try:
    adapter_upbit = importlib.import_module("src.10_trade.oems.adapter_upbit")
except Exception:
    logger.exception("[engine] adapter_upbit 모듈 로드 실패")
    adapter_upbit = None  # type: ignore

try:
    ledger = importlib.import_module("src.10_trade.oems.ledger")
except Exception:
...
async def _cli_test(timescale_dsn: Optional[str] = None):
    ...
    try:
        ...
        if router is None:
            raise RuntimeError("router 모듈이 로드되지 않았습니다. src.10_trade.oems.router 파일을 확인하세요.")
