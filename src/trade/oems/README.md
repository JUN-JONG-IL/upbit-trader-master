# src/trade/oems — OEMS (Order Execution & Management System) 책임 및 템플릿
...
권장 디렉터리/파일 (초기)
- src/trade/oems/__init__.py
- src/trade/oems/risk.py           — 리스크 체크(동기/비동기 인터페이스)
- src/trade/oems/router.py         — 주문 라우팅(SOR) / 분할
- src/trade/oems/ledger.py         — 원장(orders_ledger) 기록 / idempotent insert
- src/trade/oems/adapter_upbit.py  — 거래소 어댑터(REST/WebSocket) 샘플(추후)
- README.md (이 파일)
