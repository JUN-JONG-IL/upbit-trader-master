# -*- coding: utf-8 -*-
"""
03_market 패키지 초기화 파일 (안전화된 lazy import)

설명:
- 이 파일은 패키지 import 시 서브모듈의 전역 초기화(부작용)를 즉시 발생시키지 않도록
  모든 위젯 클래스를 지연 로드(lazy import) 방식으로 노출합니다.
- widget_factory 등에서 "import src.03_market" 형태로 패키지를 로드할 때
  하위 패키지의 __init__.py 코드가 실행되어 문제를 일으키는 것을 방지합니다.
- 실제 클래스를 사용하려 할 때(예: from src.03_market import CoinlistWidget 또는
  import src.03_market; src.03_market.CoinlistWidget) 해당 서브모듈을 동적으로 import 합니다.
...
# __all__을 명시하여 from src.03_market import * 동작을 제어
...
def __getattr__(name: str) -> Any:
    ...
    if name in _EXPORT_MODULES:
        ...
        try:
            ...
            logger.exception("03_market 패키지에서 '%s' 로드 실패", name)
