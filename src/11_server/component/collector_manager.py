...
- ê¸°ì¡´: UpbitWebSocket ?°ì„  ?¬ìš©
- ë³€ê²? UpbitWebSocket???†ìœ¼ë©?REST ?´ë§ ê¸°ë°˜ UpbitRestCollectorë¥??€???¬ìš©?˜ë„ë¡???
"""
...
# ?œë„ ?œì„œ: ?¨í‚¤ì§€ëª??•íƒœë¡?import ?œë„ (?˜ê²½???°ë¼ ?¬ë¼ì§?
_try_names = [
...
# UpbitWebSocket???†ìœ¼ë©?REST ?´ë§ collector ?œë„
if UpbitWebSocket is None:
    try:
        ...
        if UpbitRestCollector:
            logger.info("Fallback: UpbitRestCollector ?¬ìš©(REST ?´ë§)")
    except Exception:
        try:
            ...
            if candidate.exists():
                ...
                if UpbitRestCollector:
                    logger.info("Fallback file-load: UpbitRestCollector ?¬ìš©")
        except Exception:
            logger.debug("UpbitRestCollector ?Œì¼ ë¡œë“œ ?¤íŒ¨", exc_info=True)
    ...
    async def _run_upbit_ws(self, symbols: List[str]) -> None:
        ...
        if UpbitRestCollector is not None:
            try:
                ...
                logger.exception("UpbitRestCollector ?¤í–‰ ?¤íŒ¨: %s", exc)
            return

        logger.error("Upbit collector êµ¬í˜„??ì°¾ì„ ???†ìŠµ?ˆë‹¤ (UpbitWebSocket/UpbitRestCollector ëª¨ë‘ ?†ìŒ)")
        return
        ...
        try:
            if client and hasattr(client, "stop"):
                try:
                    ...
                    logger.debug("collector stop() ?¸ì¶œ ì¤??ˆì™¸", exc_info=True)
        except Exception:
            pass
