...
class IndicatorEngine:
    ...
    def _calculate_macd(self, state: Dict[str, Any], close: float) -> Dict[str, Optional[float]]:
        ...
        signal_line = self._calculate_ema(state["signal_9"], macd_line)
        state["signal_line"] = signal_line

        if signal_line is not None:
            state["histogram"] = macd_line - signal_line
        else:
    ...
    def _calculate_bollinger_bands(self, state: Dict[str, Any], close: float) -> Dict[str, Optional[float]]:
        ...
        # Welford의 온라인 분산 알고리즘으로 O(1) 증분 계산
        state["count"] += 1
        if len(values) == period:
            old_value = values[0]
            old_mean = state["mean"]
            state["mean"] += (close - old_value) / period
            state["m2"] += (close - old_value) * ((close - state["mean"]) + (old_value - old_mean))
        else:
            old_mean = state["mean"]
            state["mean"] += (close - old_mean) / state["count"]
            state["m2"] += (close - old_mean) * (close - state["mean"])
