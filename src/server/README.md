# src/server — FastAPI REST/WebSocket 서버

## 서버 개요

`src/server`는 Upbit Trader 플랫폼의 **REST API·WebSocket 서버 계층**을 담당합니다.
...
>
> `server`는 **서버 자체 설정**(포트, CORS, 인증 등)만 관리합니다.
...
```bash
uvicorn src.server.core.fastapi_app:create_app --factory \
...
### 서버 설정 (`src/server/config/server_config.py`)
