...
def _find_config_module() -> Optional[Any]:
    ...
    1) 패키지 모듈들: 'src.core.config.config', 'core.config.config', 'core.config', 'src.core.config'
    2) 최상위 'config'
    ...
    src_dir = os.path.abspath(os.path.join(current_dir, "..", "..", ".."))  # 프로젝트 src/server/... 에서 src 루트로
    candidate_paths = [
        os.path.join(src_dir, "core", "config", "config.py"),
        os.path.join(src_dir, "core", "config.py"),
