...
def start_background_loop_and_client(config: Optional[RedisConfig] = None, wait: bool = True, timeout: float = 10.0) -> Optional[Any]:
    ...
    if not REDIS_AVAILABLE:
        LOG.warning("?좑툘  redis 誘몄꽕移?- Redis ?곌껐 遺덇? (start_background skipped)")
        return None
    if wait:
        try:
...
async def create_client(config: Optional[RedisConfig] = None) -> Optional[Any]:
    ...
    global _client
    if not REDIS_AVAILABLE:
        LOG.warning("?좑툘  redis 誘몄꽕移?- Redis ?곌껐 遺덇?")
        return None
    ...
    try:
        ...
        await _client.ping()
        LOG.info("??Redis ?곌껐 ?꾨즺 (%s:%d)", cfg.host, cfg.port)
    except Exception as e:
        LOG.error("??Redis ?곌껐 ?ㅽ뙣: %s", e)
        _client = None
    return _client
