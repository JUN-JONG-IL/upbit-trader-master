# -*- coding: utf-8 -*-
"""
redis 패키지 로컬 health_check 로더

목적:
- 프로젝트 내의 로컬 구현(src/02_data/redis/health_check.py 등)을 우선 로드하고,
  없을 경우 안전한 스텁을 제공하여 예외 노출을 방지합니다.
- 로깅은 'shim'이라는 애매한 표기를 사용하지 않고 명확한 모듈/파일 경로와 오류를 남깁니다.
"""
...
# 기본 스텁 구현 (로컬 구현 없을 때 사용)
def _stub_check_redis_connection() -> str:
    # 설정이 없을 때 회색(gray) 반환 — UI에서 '미설정'으로 보여주기 위함
    return STATUS_GRAY
...
def _find_repo_root(start: Optional[str] = None) -> str:
    """
    현재 파일 위치를 기준으로 가능한 repo 루트를 탐색한다.
    - 'src' 폴더를 발견하면 그 상위 디렉토리를 repo root로 간주
    - '.git' 디렉토리가 발견되면 그 위치를 사용
    - 실패 시 start 또는 cwd 반환
    """
    try:
...
# 후보 경로(우선순위) 탐색 및 로드 시도
try:
    repo_root = _find_repo_root()
    here = os.path.dirname(__file__)  # 패키지 디렉토리 (예: .../src/02_data/redis)
    candidates = []

    # 1) same package sibling file (가장 우선)
    candidates.append(os.path.join(here, "health_check.py"))

    # 2) common project locations relative to repo_root
    #    프로젝트 구조에 따라 여러 후보를 시도 (src/02_data/redis, src/redis, redis/)
    candidates.extend([
        os.path.join(repo_root, "src", "02_data", "redis", "health_check.py"),
        os.path.join(repo_root, "src", "02_data", "redis", "health_check.py"),  # duplicate safe
    ...
    import_attempts = ["src.02_data.redis.health_check", "02_data.redis.health_check", "redis.health_check"]
...
def check_redis_connection() -> str:
    """'green'|'red'|'gray' 중 하나 반환. 예외 발생 시 'red'로 처리."""
    try:
        return _check_fn()
    except Exception as e:
...
def health_check() -> Dict[str, Optional[str]]:
    """구조화된 health-check 결과를 반환한다. impl 필드를 보강한다."""
    try:
        res = _health_fn()
