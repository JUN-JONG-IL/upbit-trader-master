...
def _find_timescale_mod() -> Optional[Any]:
    """
    ?�러 방식?�로 timescale_redis 모듈??찾아 로드?�니??
    1) ?�한 import ?�름 ?�도
    2) ?�스 ?�리 ???�보 경로 검??
    3) ?�경변??TIMESCALE_REDIS_PATH??지?�된 경로 ?�용
    4) sys.modules???��? 로드??모듈 검??
    """
    candidates_names = [
