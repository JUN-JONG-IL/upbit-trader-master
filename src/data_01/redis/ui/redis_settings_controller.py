...
- ?덉쟾?? ?묒뾽(??젣 ??? UI?먯꽌 諛섎뱶???뺤씤/沅뚰븳 泥댄겕 ???몄텧?섏꽭??
"""
...
def _find_timescale_mod() -> Optional[Any]:
    """
    ?щ윭 諛⑹떇?쇰줈 timescale_redis 紐⑤뱢??李얠븘 濡쒕뱶?⑸땲??
    1) ?뷀븳 import ?대쫫 ?쒕룄
    2) ?뚯뒪 ?몃━ ???꾨낫 寃쎈줈 寃??
    3) ?섍꼍蹂??TIMESCALE_REDIS_PATH??吏?뺣맂 寃쎈줈 ?ъ슜
    4) sys.modules???대? 濡쒕뱶??紐⑤뱢 寃??
    """
    candidates_names = [
...
# ------------------------------
# Shim: 紐⑤뱢???놁쓣 ???덉쟾???숈옉??蹂댁옣?섎뒗 寃쎈웾 ?섑띁
# ------------------------------
class _Shim:
...
# DEFAULT_QUEUE: 紐⑤뱢?먯꽌 媛?몄삤嫄곕굹 ?대갚
