# -*- coding: utf-8 -*-
"""
StatisticsTab (View ?꾩슜, 媛뺥솕??UI 濡쒕뱶/諛붿씤??
- ?듭떖 紐⑺몴: .ui媛 濡쒕뱶?섏뼱???꾩젽(objectName)???놁뼱???붾㈃??鍮꾩뼱 蹂댁씠??臾몄젣 諛⑹?.
- ?꾨왂:
  1) load_ui_with_tab_fix ?곗꽑 ?쒕룄 (?덉쑝硫?.ui enum/xml 蹂댁젙)
  2) PyQt uic.loadUi ?대갚
  3) uic.loadUiType ?대갚 (肄붾뱶 ?앹꽦??
  4) 濡쒕뱶 ???듭떖 ?꾩젽(tabWidget_main_tabs, table_tab_X ?????놁쑝硫?findChild 湲곕컲?쇰줈 ?먮룞 諛붿씤??
- ???뚯씪? View쨌?쒓렇?먃룰꼍???ы띁留??ы븿?⑸땲?? 鍮꾩쫰?덉뒪 濡쒖쭅? Controller濡??대룞?섏꽭??
"""
    ...
    class StatisticsTab(TableCopyMixin, QWidget):
        ...
        def __init__(self, parent=None, ui_filename: Optional[str] = None):
            ...
            if not loaded or not self._has_core_widgets():
                logger.warning("[StatisticsTab] UI 濡쒕뱶 ?먮뒗 諛붿씤??遺덉셿?????먮룞 諛붿씤???쒕룄")
                self._attempt_find_widgets()
                if not self._has_core_widgets():
                    self._show_ui_warning(f"StatisticsTab UI 濡쒕뱶 ?ㅽ뙣 ?먮뒗 ?듭떖 ?꾩젽 ?꾨씫: {os.path.basename(ui_filename)}")
            ...
            for i in range(1, 8):
                ...
                try:
                    ...
                    logger.debug("[StatisticsTab] _replace_table_widget_with_view ?ㅽ뙣(tab=%s): %s", i, exc)
            ...
            try:
                if bool(self._settings.get("autostart_timer", _DEF["autostart_timer"])) and not self._timer.isActive():
                    self._timer.start()
                    logger.info("[StatisticsTab] ??대㉧ ?먮룞 ?쒖옉")
            except Exception:
                pass
            ...
            try:
                if _load_ui_with_tab_fix is not None:
                    try:
                        ...
                        logger.info("[StatisticsTab] UI 濡쒕뱶 ?깃났 (load_ui_with_tab_fix)")
                        return True
                    except Exception as e:
                        logger.debug("[StatisticsTab] load_ui_with_tab_fix ?ㅽ뙣: %s", e)
                # Try uic.loadUi
                try:
                    uic.loadUi(ui_path, self)
                    logger.info("[StatisticsTab] UI 濡쒕뱶 ?깃났 (uic.loadUi)")
                    return True
                except Exception as e:
                    logger.debug("[StatisticsTab] uic.loadUi ?ㅽ뙣: %s", e)
                # Try loadUiType fallback (generates form class)
                try:
                    form_class, base_class = uic.loadUiType(ui_path)
                    form = form_class()
                    form.setupUi(self)
                    logger.info("[StatisticsTab] UI 濡쒕뱶 ?깃났 (uic.loadUiType ?대갚)")
                    return True
                except Exception as e:
                    logger.warning("[StatisticsTab] uic.loadUiType ?대갚 ?ㅽ뙣: %s", e)
                    return False
            except Exception as exc:
                logger.exception("[StatisticsTab] UI 濡쒕뱶 以??덉쇅: %s", exc)
                return False
                ...
                if not hasattr(self, "tabWidget_main_tabs") or getattr(self, "tabWidget_main_tabs") is None:
                    ...
                    if tab is not None:
                        try:
                            ...
                            logger.debug("[StatisticsTab] tabWidget_main_tabs ?먮룞 諛붿씤????(%s)", getattr(tab, "objectName", lambda: "")())
                        except Exception:
                            pass
            except Exception:
                pass
                    ...
                    if not hasattr(self, f"table_tab_{i}") or getattr(self, f"table_tab_{i}") is None:
                        ...
                        if w is not None:
                            try:
                                ...
                                logger.debug("[StatisticsTab] table_tab_%d ?먮룞 諛붿씤??(%s)", i, w.objectName())
                            except Exception:
                                pass
                except Exception:
                    pass
                    ...
                    if not hasattr(self, f"text_log_tab_{i}") or getattr(self, f"text_log_tab_{i}") is None:
                        ...
                        if w is not None:
                            try:
                                ...
                                logger.debug("[StatisticsTab] text_log_tab_%d ?먮룞 諛붿씤??(%s)", i, w.objectName())
                            except Exception:
                                pass
                except Exception:
                    pass
        ...
        def _load_settings_file_or_defaults(self) -> None:
            ...
            try:
                ...
                logger.debug("[StatisticsTab] _load_settings_file_or_defaults ?ㅽ뙣: %s", exc)
        ...
        def _save_settings_file(self) -> None:
            try:
                ...
                logger.debug("[StatisticsTab] _save_settings_file ?ㅽ뙣: %s", exc)

        # (?댄븯 濡쒖쭅 諛?UI ?ы띁???먮낯 ?숈옉???좎? ???앸왂??遺遺꾩? ?꾩슂 ???먮낯 洹몃?濡?遺숈뿬 ?곸슜?섏꽭??)
        # 二쇱슂 ?숈옉: _replace_table_widget_with_view, _on_section_resized, set_log_handler, _register_forwarding_handler,
        # _setup_auto_log_handler, _add_bootstrap_stream_handler, add_log_entry, _on_timer_flush, _collect_filters_for_tab,
        # _update_proxy_filters, UI ?≪뀡?? export/load history, _filter_log_item, _get_max_rows_for_tab, closeEvent ??
        #
        # ?꾩껜 ?먮낯 濡쒖쭅??蹂댁〈?섎젮硫????뚯씪???섎㉧吏 硫붿꽌???ㅻ━吏??援ы쁽)瑜?洹몃?濡??⑹퀜 ?ъ슜?섏떆硫??⑸땲??
