...
RedisClient = None
try:
    # 상대 import: package 구조에서 '...redis.core.client'로 접근 (ui -> pipeline -> 02_data)
    from ...redis.core.client import RedisClient  # type: ignore
except Exception:
    try:
        # 런타임에서 패키지 경로로 로드 시도 (환경에 따라 동작)
        mod = importlib.import_module("src.02_data.redis.core.client")
        RedisClient = getattr(mod, "RedisClient", None)
    except Exception:
        # 파일경로 폴백: src/02_data/redis/core/client.py
        try:
            base = Path(__file__).resolve().parents[3]  # repo/src
            candidate = base / "02_data" / "redis" / "core" / "client.py"
            if candidate.exists():
