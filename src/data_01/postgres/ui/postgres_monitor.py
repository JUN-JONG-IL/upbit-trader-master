# -*- coding: utf-8 -*-
"""
PostgreSQL 모니터링 다이얼로그
- postgres_monitor.ui 로드
...
_UI_FILE = os.path.join(os.path.dirname(__file__), "postgres_monitor.ui")
...
if _HAS_QT:
    class PostgresMonitorDialog(QDialog):
        """
        PostgreSQL 모니터링 다이얼로그.
        postgres_monitor.ui 파일을 로드하고 tabs/*.ui 파일을 QTabWidget에 동적 추가.
        """
        ...
        def _load_ui(self):
            ...
            try:
                ...
                logger.debug("[PostgresMonitorDialog] UI 파일 로드 실패: %s", e)
            # 폴백 레이아웃
            from PyQt5.QtWidgets import (
                QPushButton, QHBoxLayout, QSpacerItem, QSizePolicy
            )
            self.setWindowTitle("PostgreSQL 모니터링")
        ...
        def _load_tabs(self):
            ...
            if tab_widget is None:
                logger.warning("[PostgresMonitorDialog] tabWidget을 찾을 수 없습니다.")
                return
            ...
            tab_defs = [
                ...
                ("event_store_tab",   "EventStoreTab",   "📦 이벤트 스토어"),
                ("ledger_tab",        "LedgerTab",       "📒 거래 원장"),
                ("replication_tab",   "ReplicationTab",  "🔄 복제 상태"),
                ("query_tab",         "QueryTab",        "📂 쿼리 로그"),
                    ...
                    try:
                        ...
                        if hasattr(widget, "start_updates"):
                            try:
                                ...
                                logger.debug("[PostgresMonitorDialog] start_updates 실패 (%s): %s", module_name, exc)
                        continue
                    except Exception as exc:
    ...
    class PostgresMonitorDialog:  # type: ignore
        """PyQt5 미설치 시 폴백 스텁"""
        def __init__(self, *args, **kwargs):
            logger.warning("[PostgresMonitorDialog] PyQt5 미설치 - 폴백 스텁")
