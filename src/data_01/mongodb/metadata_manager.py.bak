...
변경사항 요약:
    ...
    - create_metadata_manager(...) 팩토리 확장: 다양한 인자 이름 허용(db/mongo_db/mongo/data_manager/static 등),
      내부적으로 src.02_data.mongodb.init_mongodb.get_db()를 시도해 동기 DB 획득을 지원하도록 보강.
...
def _try_get_db_via_init_module() -> Optional[Any]:
    ...
    실패하면 None.
    """
    candidates = ("src.02_data.mongodb.init_mongodb", "02_data.mongodb.init_mongodb", "02_data.mongodb.init_mongodb")
