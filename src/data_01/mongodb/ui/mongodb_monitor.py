...
_UI_FILE = os.path.join(os.path.dirname(__file__), "mongodb_monitor.ui")
...
if _HAS_QT:
    class MongoDBMonitorDialog(QDialog):
        """
        MongoDB 모니터링 다이얼로그.
        mongodb_monitor.ui 파일을 로드하고 tabs/*.ui 파일을 QTabWidget에 동적 추가.
        """
        ...
        def _load_ui(self):
            ...
            try:
                ...
                logger.debug("[MongoDBMonitorDialog] UI 파일 로드 실패: %s", e)
            # 폴백 레이아웃
            from PyQt5.QtWidgets import (
                QPushButton, QHBoxLayout, QSpacerItem, QSizePolicy
            )
            self.setWindowTitle("MongoDB 모니터링")
        ...
        def _load_tabs(self):
            ...
            if tab_widget is None:
                logger.warning("[MongoDBMonitorDialog] tabWidget을 찾을 수 없습니다.")
                return
            ...
            tab_defs = [
                ...
                ("metadata_tab",  "MetadataTab",  "🗂️ 메타데이터"),
                ("query_tab",     "QueryTab",     "📂 쿼리 로그"),
                ("settings_tab",  "SettingsTab",  "⚙️ 설정 관리"),
                    ...
                    try:
                        ...
                        if hasattr(widget, "start_updates"):
                            try:
                                ...
                                logger.debug("[MongoDBMonitorDialog] start_updates 실패 (%s): %s", module_name, exc)
                        continue
                    except Exception as exc:
    ...
    class MongoDBMonitorDialog:  # type: ignore
        """PyQt5 미설치 시 폴백 스텁"""
        def __init__(self, *args, **kwargs):
            logger.warning("[MongoDBMonitorDialog] PyQt5 미설치 - 폴백 스텁")
