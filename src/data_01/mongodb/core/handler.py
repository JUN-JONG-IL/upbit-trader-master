"""
?¥Îçî Î™©Ï†Å: ?∞Ïù¥??Í≥ÑÏ∏µ(MongoDB/Motor, Redis, SQLAlchemy ?? ?µÌï© IO ?∏Îì§?? 
?ÅÏö© ?§ÌÉù: Python 3.11, motor, redis, sqlalchemy, orjson, dask, polars, pandas  
[Purpose]
- NoSQL/SQL/Ï∫êÏãú Í≤ΩÏú† ?∞Ïù¥???ÖÏ∂ú?? ?Ä?©Îüâ ÏµúÏ†Å?? Ï∫êÏãú ?∞Îèô, Î∂ÑÏÇ∞Ï≤òÎ¶¨ ÏßÄ??
"""
import logging
import asyncio
import importlib
...
def _get_redis_url() -> str:
    """config.yaml Í∏∞Î∞ò Redis URL Î∞òÌôò (fallback: ?¨Ìä∏ 58530, password=dummy)"""
...
class DBHandler:
    ...
    def __init__(self,
        ...
        # RDB ?îÏßÑ(SQLAlchemy, ?àÏãú: SQLite, ?ïÏû• Í∞Ä??
        self.engine = sqlalchemy.create_engine('sqlite:///trade.db')
        # Redis ?¥Îùº?¥Ïñ∏??(config.yaml Í∏∞Î∞ò)
    ...
    async def ensure_index(self, db_name: str, collection_name: str):
        """collection??time ?∏Îç±???ùÏÑ±(?±Îä• ÏµúÏ†Å??"""
    ...
    async def insert_item_one(self, data: dict = None, db_name: str = None, collection_name: str = None) -> str:
        """Document 1Í∞??Ä?? RDB/Redis ?ôÏãú ?ôÍ∏∞??""
        ...
        # RDB (?àÏãú) - convert to pandas for SQL append
        try:
    ...
    async def insert_item_many(self, data: list, db_name: str, collection_name: str, ordered: bool = False) -> list:
        """Document ?§Ïàò ?Ä?? RDB/Redis??Î≥ëÎ†¨ Ï≤òÎ¶¨ (batching)."""
        await self.ensure_index(db_name, collection_name)
        batch_size = 1000
        results = []
    ...
    async def find_item_one(self, condition: dict = None, db_name: str = None, collection_name: str = None) -> dict:
        """Ï∫êÏãú ?∞ÏÑ†, ?ÜÏúºÎ©?DB?êÏÑú 1Í∞?Ï°∞Ìöå"""
    ...
    async def find_item(self, condition: dict = None, db_name: str = None, collection_name: str = None):
        """?Ä??Ï°∞Ìöå, Dask DFÎ°?Î∂ÑÏÇ∞ Î≥Ä??(?ÜÏúºÎ©?pandas DataFrame Î∞òÌôò)"""
...
async def main():
    ...
    # ?àÏãú: 'candles' DB??'KRW-ADA_minute_1' Ïª¨Î†â?òÏóê???ÑÏ≤¥(?¥Î¶ºÏ∞®Ïàú)
