...
- 변경 요지:
  * 외부/로컬 모듈 로드를 안전한 런타임 로더로 수행하여
    'src.02_data' 형태의 식별자 때문에 발생하는 SyntaxError를 회피합니다.
...
try:
    ...
    # 프로젝트 내 파일 경로로 시도: src/02_data/timescale/ui/timescale_db.py 또는 src/02_data/timescale/timescale_db.py
    base = Path(__file__).resolve().parents[3]  # .../src/02_data/timescale/ui -> go up to repo/src
    candidates = [
        base / "02_data" / "timescale" / "ui" / "timescale_db.py",
        base / "02_data" / "timescale" / "timescale_db.py",
    ]
...
try:
    ...
    candidate = base / "02_data" / "timescale" / "timescale_redis.py"
    if candidate.exists():
...
try:
    ...
    gm = importlib.import_module("src.02_data.pipeline.ui.gap_monitor_dialog")
    GapMonitorDialog = getattr(gm, "GapMonitorDialog", None)
except Exception:
    # 2) 파일 경로로 로드: ../pipeline/ui/gap_monitor_dialog.py
    base = Path(__file__).resolve().parents[3]
    candidate = base / "02_data" / "pipeline" / "ui" / "gap_monitor_dialog.py"
    if candidate.exists():
