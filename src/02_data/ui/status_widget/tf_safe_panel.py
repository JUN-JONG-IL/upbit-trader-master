...
if _HAS_QT:
    ...
    class _TFSafeWorker(QThread):
        ...
        def _resolve_metadata_manager() -> Optional[Any]:
            ...
            ``02_data`` 패키지명이 숫자로 시작해 일반 ``import_module`` 가 불가하므로
            ...
            try:
                ...
                # tf_safe_panel.py: src/02_data/ui/status_widget/  →  parents[3] == src/
                src_root = here.parents[3]
                mm_path = src_root / "02_data" / "mongodb" / "metadata_manager.py"
    ...
    class TFSafePanelMixin:
        ...
        def _init_tf_safe_panel(self) -> None:
            ...
            try:
                # PyQt5 위젯도 `02_data` 디지트 프리픽스 패키지 안에 있어
                ...
                tfp_path = src_root / "02_data" / "ui" / "widgets" / "tf_progress_widget.py"
