# -*- coding: utf-8 -*-
"""
ClickHouse 모니터링 다이얼로그
- clickhouse_monitor.ui 로드
...
_UI_FILE = os.path.join(os.path.dirname(__file__), "clickhouse_monitor.ui")
...
if _HAS_QT:
    class ClickHouseMonitorDialog(QDialog):
        """
        ClickHouse 모니터링 다이얼로그.
        clickhouse_monitor.ui 파일을 로드하고 tabs/*.ui 파일을 QTabWidget에 동적 추가.
        """
        ...
        def _load_ui(self):
            ...
            try:
                ...
                logger.debug("[ClickHouseMonitorDialog] UI 파일 로드 실패: %s", e)
            # 폴백 레이아웃
            from PyQt5.QtWidgets import (
                QPushButton, QHBoxLayout, QSpacerItem, QSizePolicy
            )
            self.setWindowTitle("ClickHouse 모니터링")
        ...
        def _load_tabs(self):
            ...
            if tab_widget is None:
                logger.warning("[ClickHouseMonitorDialog] tabWidget을 찾을 수 없습니다.")
                return
            ...
            tab_defs = [
                ...
                ("performance_tab", "PerformanceTab",  "📈 성능 모니터링"),
                ("overview_tab",    "OverviewTab",     "📂 테이블 관리"),
                ("merge_tab",       "MergeTab",        "🔄 Merge 상태"),
                ("query_tab",       "QueryTab",        "📊 쿼리 성능"),
                    ...
                    try:
                        ...
                        if hasattr(widget, "start_updates"):
                            try:
                                ...
                                logger.debug("[ClickHouseMonitorDialog] start_updates 실패 (%s): %s", module_name, exc)
                        continue
                    except Exception as exc:
    ...
    class ClickHouseMonitorDialog:  # type: ignore
        """PyQt5 미설치 시 폴백 스텁"""
        def __init__(self, *args, **kwargs):
            logger.warning("[ClickHouseMonitorDialog] PyQt5 미설치 - 폴백 스텁")
