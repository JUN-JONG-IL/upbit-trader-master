...
# NOTE: This module is part of the local 'redis' package (src/02_data/redis/).
# When src/02_data/ is on sys.path, 'import redis.asyncio' would resolve to this
...
def _load_external_redis_asyncio():
    ...
    """
    import os
    # Our own package directory (src/02_data/redis/)
...
def start_background_loop_and_client(config: Optional[RedisConfig] = None, wait: bool = True, timeout: float = 10.0) -> Optional[Any]:
    ...
    if not REDIS_AVAILABLE:
        LOG.warning("⚠️  redis 미설치 - Redis 연결 불가 (start_background skipped)")
        return None
    if wait:
        try:
...
async def create_client(config: Optional[RedisConfig] = None) -> Optional[Any]:
    ...
    global _client
    if not REDIS_AVAILABLE:
        LOG.warning("⚠️  redis 미설치 - Redis 연결 불가")
        return None
    ...
    try:
        ...
        await _client.ping()
        LOG.info("✅ Redis 연결 완료 (%s:%d)", cfg.host, cfg.port)
    except Exception as e:
        LOG.error("❌ Redis 연결 실패: %s", e)
        _client = None
    return _client
