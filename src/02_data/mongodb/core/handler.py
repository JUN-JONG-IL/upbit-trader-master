"""
폴더 목적: 데이터 계층(MongoDB/Motor, Redis, SQLAlchemy 등) 통합 IO 핸들러  
적용 스택: Python 3.11, motor, redis, sqlalchemy, orjson, dask, polars, pandas  
[Purpose]
- NoSQL/SQL/캐시 경유 데이터 입출력, 대용량 최적화, 캐시 연동, 분산처리 지원
"""
import logging
import asyncio
import importlib
...
# Import the PyPI 'redis' package, bypassing the local 'redis' package in src/02_data/.
# The local redis package (src/02_data/redis/) would otherwise shadow the PyPI package
# when src/02_data/ is on sys.path.
def _load_external_redis():
    ...
    data_dir = os.path.dirname(our_data_dir)  # src/02_data/
...
class DBHandler:
    ...
    def __init__(self,
        ...
        # RDB 엔진(SQLAlchemy, 예시: SQLite, 확장 가능)
        self.engine = sqlalchemy.create_engine('sqlite:///trade.db')
        # Redis 클라이언트 (config.yaml 기반)
    ...
    async def ensure_index(self, db_name: str, collection_name: str):
        """collection에 time 인덱스 생성(성능 최적화)"""
    ...
    async def insert_item_one(self, data: dict = None, db_name: str = None, collection_name: str = None) -> str:
        """Document 1개 저장, RDB/Redis 동시 동기화"""
        ...
        # RDB (예시) - convert to pandas for SQL append
        try:
    ...
    async def insert_item_many(self, data: list, db_name: str, collection_name: str, ordered: bool = False) -> list:
        """Document 다수 저장, RDB/Redis도 병렬 처리 (batching)."""
        await self.ensure_index(db_name, collection_name)
        batch_size = 1000
        results = []
    ...
    async def find_item(self, condition: dict = None, db_name: str = None, collection_name: str = None):
        """대량 조회, Dask DF로 분산 변환 (없으면 pandas DataFrame 반환)"""
...
async def main():
    ...
    # 예시: 'candles' DB의 'KRW-ADA_minute_1' 컬렉션에서 전체(내림차순)
