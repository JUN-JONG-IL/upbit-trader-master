#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Continuous Aggregates 탭 컨트롤러 (QThread Worker 패턴)
...
_UI_PATH = os.path.join(os.path.dirname(__file__), "cagg_tab.ui")

_CAGG_QUERY = """
    SELECT
        ca.view_name,
        ca.hypertable_name                                          AS source_table,
        COALESCE(j.schedule_interval::text, '정책 없음')            AS refresh_interval,
        COALESCE(
            (SELECT MAX(range_end)::text
               FROM timescaledb_information.chunks c2
              WHERE c2.hypertable_name = ca.materialization_hypertable_name),
            '-'
        )                                                           AS latest_range
...
class CaggTab(QWidget if _HAS_QT else object):
    """Continuous Aggregates 탭.

    TimescaleDB 연속 집계 뷰의 목록, 원본 테이블, 갱신 주기, 최신 시각을
    ...
    def __init__(self, db_conn=None, conn_params: Optional[Dict] = None, parent=None):
        ...
        try:
            ...
            logger.warning("[CaggTab] UI 로드 실패: %s", exc)
    ...
    def _setup_table(self):
        """테이블 위젯 초기 설정."""
        tbl = getattr(self, "table_cagg", None)
        if tbl is None:
            return
    ...
    def _update(self):
        ...
        if lbl:
            lbl.setText("상태: 조회 중...")
        self._worker = TimescaleWorker(self._conn_params, _CAGG_QUERY)
    ...
    def _on_data_ready(self, rows) -> None:
        ...
        if not rows:
            tbl.setRowCount(1)
            tbl.setItem(0, 0, QTableWidgetItem("(연속 집계 뷰 없음)"))
        ...
        if lbl:
            ...
            lbl.setText(f"✅ 마지막 갱신: {now}  |  {len(rows)} 개 CAGG")
    ...
    def _on_error(self, msg: str) -> None:
        ...
        logger.debug("[CaggTab] Worker 오류: %s", msg)
