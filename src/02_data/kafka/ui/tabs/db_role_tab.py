# -*- coding: utf-8 -*-
"""Kafka 역할 안내 탭 (정적 정보 표시, DB 연결 불필요)

역할:
- Kafka의 플랫폼 내 역할, 저장 데이터, 특화 기능, 활용 방안을 표시
...
if _HAS_QT:
    class DbRoleTab(QWidget):
        ...
        Kafka의 역할, 저장 데이터, 특화 기능, 활용 방안을 정적으로 표시합니다.
        ...
        def _build_fallback_ui(self) -> None:
            ...
            title = QLabel("🟠 Kafka — 실시간 이벤트 스트리밍 파이프라인")
            title.setStyleSheet(
                "font-size: 14pt; font-weight: bold; "
                "color: #FFFFFF; background-color: #EA580C; padding: 12px; border-radius: 6px;"
            )
            content_layout.addWidget(title)

            body_text = (
                "<b>📌 역할</b><br>"
                "Kafka는 분산 이벤트 스트리밍 플랫폼입니다.<br>"
                "포트: 9092 | 응답: &lt; 5ms | 보관: 24시간~7일<br><br>"

                "<b>💾 저장 데이터</b><br>"
                "• upbit.candles.1m / 5m / 1h — 캔들 데이터 스트림<br>"
                "• upbit.ticks — 실시간 체결 데이터 스트림<br>"
                "• upbit.orderbook — 호가 데이터 스트림<br>"
                "• upbit.events — 시스템 이벤트<br>"
                "• db.timescale.archive — 이관용 아카이브 토픽<br><br>"

                "<b>⚙️ 특화 기능</b><br>"
                "• Topic 파티셔닝 (hash(symbol) % 16)<br>"
                "• Consumer Group 병렬 처리<br>"
                "• 메시지 보관 정책 (24시간~7일)<br>"
                "• Snappy 압축 (70% 절감)<br><br>"

                "<b>🚀 활용 방안</b><br>"
                "실시간 데이터 파이프라인 · 장애 복구(Replay) · 마이크로서비스 디커플링"
            )
