...
```
src/06_ai/
?쒋?? core/                        # Central orchestration
??  ?쒋?? engine.py                # AIEngineManager ??main orchestrator
??  ?쒋?? model_registry.py        # MLflow model version management
??  ?붴?? inference.py             # Real-time inference engine (<100 ms)
?쒋?? prediction/                  # Price & anomaly prediction
??  ?쒋?? price_predictor.py       # LSTM / BiLSTM / Transformer / XGBoost
??  ?쒋?? pattern_detector.py      # Candlestick & chart pattern recognition
??  ?붴?? anomaly_detector.py      # Autoencoder anomaly detection
?쒋?? sentiment/                   # Sentiment analysis
??  ?쒋?? news_analyzer.py         # Financial news sentiment (FinBERT)
??  ?붴?? social_analyzer.py       # Twitter / Reddit sentiment
?쒋?? strategy/                    # Strategy recommendation
??  ?쒋?? recommender.py           # Market-regime-aware strategy ranking
??  ?붴?? optimizer.py             # Optuna hyperparameter optimisation
?쒋?? utils/                       # Shared utilities
??  ?쒋?? feature_engineering.py   # Technical indicators, sequences
??  ?붴?? preprocessing.py         # Scaling, splitting, cleaning
?쒋?? ai_engine/                   # LLM assistant & monitoring
??  ?붴?? ml_service.py            # MLService ??Gap/Anomaly ML model selector (moved from priority/)
?쒋?? detection/                   # Pump & dump anomaly detection
?쒋?? models/                      # Base predictors (LSTM, Transformer, XGB)
?쒋?? prediction/                  # Prediction UI & ensemble logic
?쒋?? priority/                    # Priority scoring & settings (AI/ML 遺꾨━ ??
??  ?쒋?? services/
??  ??  ?쒋?? priority_service.py      # ?곗꽑?쒖쐞 ?먯닔 怨꾩궛 (?쒖옄由?
??  ??  ?쒋?? priority_db_service.py   # ?곗꽑?쒖쐞 DB CRUD (?쒖옄由?
??  ??  ?쒋?? ml_service.py            # [SHIM] ??ai_engine/ml_service.py
??  ??  ?붴?? upbit_data_provider.py   # [SHIM] ??src/data_01/clients/
??  ?붴?? ui/
??      ?쒋?? priority_settings.ui     # ?곗꽑?쒖쐞 ?ㅼ젙 UI (?쒖옄由?
??      ?쒋?? priority_dashboard.ui    # ?곗꽑?쒖쐞 ??쒕낫??UI (?쒖옄由?
??      ?붴?? ml_model_selector.ui     # [MOVED] ??ui/ai_engine/ml_model_selector.ui
?쒋?? ui/
??  ?붴?? ai_engine/
??      ?붴?? ml_model_selector.ui     # ML 紐⑤뜽 ?좏깮 UI (priority/ui/ ?먯꽌 ?대룞)
?쒋?? rl/                          # Reinforcement learning (PPO, DQN)
