# CHANGELOG
# 2026-03-16 | Copilot | 업그레이드: sentiment README v4.0. 버전 통일.
# 2026-03-13 | Copilot | 업그레이드: sentiment README v2.0. 전체 템플릿(개요/구조/기능/예시/의존성/참고) 추가.
# 2026-03-06 | Copilot | 생성: sentiment README 초안
...
- **소스 필터링**: 소스별·키워드별 필터링
- **신호 생성**: 감성 급등/급락 탐지 → `core/events/` 이벤트 버스 발행
- **MongoDB 저장**: 감성 데이터 및 원본 텍스트 영구 저장

## 사용 예시

```python
from src._sentiment import SentimentWidget, SentimentEngine
...
# 뉴스 수집 워커
from src._sentiment.analysis.workers import NewsWorker
...
- `src/core/` : 이벤트 버스 (신호 발행), 설정 관리
- `src/data_01/mongodb/` : 감성 데이터 저장 (MongoDB)
- `src/data_01/redis/` : 실시간 감성 캐시
