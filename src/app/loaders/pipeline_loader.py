...
# -*- coding: utf-8 -*-
"""
Pipeline ?µÌï© Î°úÎçî (?ïÏÑù??Í≤ΩÎ°ú ?∞ÏÑ†, shim ?úÍ±∞)

Ï£ºÏöî Î≥ÄÍ≤?Î≥¥ÏôÑ:
- src Î£®Ìä∏ Í≥ÑÏÇ∞????Í≤¨Í≥†?òÍ≤å ??(?ÅÏúÑ?êÏÑú 'src' ?îÎ†â?†Î¶¨ Î∞úÍ≤¨???∞ÏÑ†).
- ?åÏùºÎ°?Î°úÎìú??Î™®Îìà??sys.modules???±Î°ù?òÏó¨ ?¥Î? import ?àÏ†ï??Î≥¥Í∞ï.
- static.log ?∞ÏÑ† ?¨Ïö© ?†Ï?, ?ÜÏúºÎ©?Î™®Îìà Î°úÍ±∞ ?êÎäî print ?¥Î∞±.
- repo-wide heavy search ?úÍ±∞ (?±Îä•/?∏Ïù¥Ï¶?Î¨∏Ï†ú ?åÌîº).
- Noop ?ÄÏ≤¥Îäî ?†Ï?(PIPELINE_ALLOW_NOOP=True Í∏∞Î≥∏)?òÏó¨ ?àÏ†Ñ?òÍ≤å ?±Ïù¥ ?ôÏûë?òÎèÑÎ°???
- ??Stager/Finalizer Ï£ºÍ∏∞??flush ?êÎèô ?úÏûë Ï∂îÍ?
- ??Timescale Í¥Ä??Î™®Îìà?êÏÑú get_timescale_connector() ?∏Ï∂ú???úÎèÑ?òÏó¨
      connector/?ÑÏó≠?Ä??pg_pool ?ÑÎ≥¥Î°??¨Ïö©?òÎäî Î°úÏßÅ??Î≥¥Í∞ï.
"""
...
def _log(static: Any, level: str, msg: str, *args) -> None:
    """static.logÍ∞Ä ?àÏúºÎ©??¨Ïö©, ?ÜÏúºÎ©?Î™®Îìà Î°úÍ±∞ ?êÎäî print ?¥Î∞±."""
    try:
...
def _short_list(iterable: Iterable, limit: int = 20) -> List[Any]:
    """?îÎ≤ÑÍ∑?Ï∂úÎ†•???ÑÌï¥ Î¶¨Ïä§?∏Î? ÏßßÍ≤å Ï§ÑÏûÑ."""
    try:
        lst = list(iterable)
        if len(lst) > limit:
...
def _load_module_from_file(path: str, alias: str) -> Optional[Any]:
    """
    ?åÏùº Í≤ΩÎ°ú?êÏÑú Î™®Îìà Î°úÎìú ?úÎèÑ. ?±Í≥µ ??sys.modules??aliasÎ°??±Î°ù?òÏó¨
    Î™®Îìà ?¥Î????ÅÎ?/?àÎ? importÍ∞Ä ?àÏ†ï?ÅÏúºÎ°??ôÏûë?òÎèÑÎ°???
    """
    try:
...
def _try_candidates_with_file(candidates: Iterable[str], file_relpath: Optional[str], static: Any, src_root: str) -> Optional[Any]:
    """
    ?ÑÎ≥¥ ?§ÏûÑ?§Ìéò?¥Ïä§?§ÏùÑ ?úÏ∞® import ?úÎèÑ?òÍ≥†, ?§Ìå®?òÎ©¥ src_root + file_relpath Î•?file-load ?úÎèÑ.
    file_relpath?Ä src Î£®Ìä∏ Í∏∞Ï? ?ÅÎ?Í≤ΩÎ°ú(?? 'data_01/pipeline/processor.py').
    """
    tried: List[Tuple[str, str]] = []

    # 1) namespace imports (?ïÏÑù???úÏÑú)
    ...
    # 2) file-level load from src root (?ïÏÑù?? ?åÏä§ ?∞ÏÑ†)
    if file_relpath:
        try:
...
# -----------------------------
# A) pg_pool ?êÏÉâ ?êÎèô??(Í∞ïÌôî)
# -----------------------------
def _discover_pg_pool(static: Any) -> Optional[Any]:
    """
    static Î∞?static.data_manager?êÏÑú Í∞Ä?•Ìïú pg_pool ?ÑÎ≥¥?§ÏùÑ Ï∞æÏïÑ Î∞òÌôò.
    ?êÌïú timescale Í¥Ä??Î™®Îìà?êÏÑú get_timescale_connector() ?∏Ï∂ú???úÎèÑ?òÏó¨
    connector/?ÑÏó≠?Ä???ÑÎ≥¥Î°??¨Ìï®?©Îãà??
    Î∞òÌôòÍ∞íÏ? 'pool' ?ïÌÉú ?êÎäî Connector Í∞ùÏ≤¥(?§Ìñâ/Ïª§Î∞ã/ executemany ÏßÄ??Î•??àÏö©?©Îãà??
    """
    candidates = [
    ...
    # 3) timescale Í¥Ä??Î™®Îìà?êÏÑú connector ?ªÍ∏∞ ?úÎèÑ (Í∞ïÌôî)
    try:
        try:
            ts_mod = importlib.import_module("data_01.timescale.timescale_db")
        except Exception:
            # ?åÏùº ?àÎ≤®Î°?ÏßÅÏ†ë Î°úÎìú ?úÎèÑ (ÎπÑÌå®?§Ï? ?§Ìñâ ?òÍ≤Ω)
        ...
        if ts_mod is not None:
            # ?∞ÏÑ†, Î™®Îìà ?¥Î??êÏÑú ?¥Î? pool/pg_pool Í∞ôÏ? ?¨Î≥º???àÎäîÏßÄ Ï≤¥ÌÅ¨
            for name in ("pg_pool", "pool", "timescale_pool"):
                try:
                    ...
                    continue
            # ?§Ïùå?ºÎ°ú get_timescale_connector ?úÎèÑ (?êÎèô Ï¥àÍ∏∞???¨Ìï®)
    ...
    # 4) module-level minimal fallbacks (?àÏ†Ñ Î∞©Ïãù ?†Ï?)
...
# -----------------------------
# Noop ?ÄÏ≤??¥Îûò?§Îì§ (ÏµúÏÜå ?ôÏûë Î≥¥Ïû•)
# -----------------------------
class NoopWriter:
...
# -----------------------------
# ?¨Ìçº: ?∏Ïä§?¥Ïä§??/ ?©ÌÜ†Î¶??∏Ï∂ú (Í∏∞Ï°¥ Î°úÏßÅ ?†Ï?)
# -----------------------------
...
# -----------------------------
# main setup_pipeline (?ïÏÑù??
# -----------------------------
def _find_src_root(start: str) -> str:
    """
    start Í≤ΩÎ°ú?êÏÑú ?ÑÎ°ú ?¨ÎùºÍ∞ÄÎ©?'src' ?îÎ†â?†Î¶¨Î•?Ï∞æÎäî?? Î™?Ï∞æÏúºÎ©?start???ÅÏúÑ 3?®Í≥Ñ ?îÎ†â?†Î¶¨Î•??¨Ïö©.
    """
    ...
    for _ in range(6):
        ...
        if parent == p:
            break
        p = parent
    # fallback: ?ÅÏúÑ 3?®Í≥ÑÎ•?src_rootÎ°?Í∞ÑÏ£º
    return os.path.abspath(os.path.join(start, "..", "..", ".."))


def setup_pipeline(static: Any) -> None:
    """
    Pipeline Íµ¨ÏÑ±?îÏÜåÎ•??ôÏ†Å?ºÎ°ú Î°úÎìú?òÍ≥† PipelineProcessor ?∏Ïä§?¥Ïä§Î•??ùÏÑ±?òÏó¨
    static.processor???±Î°ù?©Îãà??

    ?§Í≥Ñ ?êÏπô: shim/ÎπÑÏ†ï???ÑÎ≥¥ ?úÍ±∞, src Î£®Ìä∏ Í∏∞Ï????úÏ? Í≤ΩÎ°ú ?∞ÏÑ†, ?åÏùº ?¥Îèô Í∏àÏ?.
    """
    ...
    try:
        ...
        # --------------------
        # PipelineProcessor Î°úÎìú (?ïÏÑù Í≤ΩÎ°úÎß?
        # --------------------
        ...
        # --------------------
        # Î¶¨ÏÜå???òÏßë (pg_pool, redis, kafka, data_manager)
        # --------------------
        ...
        # --------------------
        # MetadataManager (?ïÏÉÅ Í≤ΩÎ°ú ?∞ÏÑ†, ?åÏùº ?¥Î∞±)
        # --------------------
        metadata = None
        ...
        # --------------------
        # CandleWriter (?ïÏÑù Í≤ΩÎ°ú + file load)
        # --------------------
        writer = None
        CandleWriter = None
        ...
        # ??========================================
        # ??Stager Ï£ºÍ∏∞??flush ?úÏûë (30Ï¥àÎßà??
        # ??========================================
            ...
            try:
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        _log(static, "info", "[pipeline_loader] CandleStager Ï£ºÍ∏∞??flush ?úÏûë (30Ï¥?")
                    else:
                        _log(static, "info", "[pipeline_loader] ?¥Î≤§??Î£®ÌîÑ ÎØ∏Ïã§??- Stager Ï£ºÍ∏∞??flush ?§ÌÇµ")
                except RuntimeError:
                    _log(static, "info", "[pipeline_loader] ?¥Î≤§??Î£®ÌîÑ ?ÜÏùå - Stager Ï£ºÍ∏∞??flush ?§ÌÇµ")
            except Exception as e:
                _log(static, "warning", "[pipeline_loader] Stager Ï£ºÍ∏∞??flush ?úÏûë ?§Ìå®: %s", e)
        else:
            if stager is None:
                _log(static, "debug", "[pipeline_loader] StagerÍ∞Ä None - Ï£ºÍ∏∞??flush ?§ÌÇµ")
            elif not hasattr(stager, "start_periodic_flush"):
                _log(static, "debug", "[pipeline_loader] Stager??start_periodic_flush Î©îÏÑú???ÜÏùå")

        # ??========================================
        # ??Finalizer Ï£ºÍ∏∞??flush ?úÏûë (Í∏∞Ï°¥ Î°úÏßÅ)
        # ??========================================
            ...
            try:
                ...
                try:
                    ...
                    if loop.is_running():
                        ...
                        _log(static, "info", "[pipeline_loader] CandlesFinalizer Ï£ºÍ∏∞??flush ?úÏûë")
                    else:
                        _log(static, "info", "[pipeline_loader] ?¥Î≤§??Î£®ÌîÑ ÎØ∏Ïã§??- Finalizer Ï£ºÍ∏∞??flush ?§ÌÇµ")
                except RuntimeError:
                    _log(static, "info", "[pipeline_loader] ?¥Î≤§??Î£®ÌîÑ ?ÜÏùå - Finalizer Ï£ºÍ∏∞??flush ?§ÌÇµ")
            except Exception as e:
                _log(static, "warning", "[pipeline_loader] Finalizer Ï£ºÍ∏∞??flush ?úÏûë ?§Ìå®: %s", e)
        ...
        try:
            ...
            if realtime_manager is not None:
                ...
                _log(static, "info", "[pipeline_loader] RealtimeManager ?±Î°ù ?ÑÎ£å: realtime_manager, rt_manager, manager")
            else:
                _log(static, "warning", "[pipeline_loader] ?†Ô∏è static.chart is None - RealtimeManagerÎ•??±Î°ù?????ÜÏäµ?àÎã§")
        except Exception as e:
            _log(static, "warning", "[pipeline_loader] RealtimeManager ?±Î°ù ?§Ìå®: %s", e)
