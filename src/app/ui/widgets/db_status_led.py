...
# -*- coding: utf-8 -*-
"""
DB ?°ê²° ?íƒœ LED ?„ì ¯ (ìºì‹œ??ëª¨ë“ˆ ë¡œë“œë¡??´ë§ ë¹„ìš©/ë¡œê·¸ ê°ì†Œ)
"""
...
# ?´ëž˜??ëª¨ë“ˆ ì°¨ì›??ìºì‹œ: ??ë²?ë°œê²¬??health_check ëª¨ë“ˆ?€ ?¬ì‚¬??
...
class DBStatusLED(QLabel):  # type: ignore[misc]
    def __init__(self, db_name: str, parent=None) -> None:
        ...
        self.setToolTip(f"{db_name}: ?•ì¸ ì¤?..")
        # ìºì‹œ ???? "redis")ë¡?health module??ì°¾ìŒ
    ...
    def _ensure_data_dir_in_sys_path(self) -> None:
        """??ë²ˆë§Œ ?¤í–‰: src/data_01 ê²½ë¡œë¥?sys.path??ì¶”ê?(ì¤‘ë³µ ?½ìž… ?Œí”¼)."""
        try:
            here = os.path.dirname(os.path.abspath(__file__))
            repo_root = os.path.abspath(os.path.join(here, "..", "..", ".."))
            data_dir = os.path.join(repo_root, "src", "data_01")
            # ê¸°ì¡´ ê°’ë“¤ê³?ì¤‘ë³µ?????ˆìœ¼ë¯€ë¡???ë²ˆë§Œ ì¶”ê?
    ...
    def _apply_style(self, status: str) -> None:
        ...
        try:
            ...
            logger.debug("[DBStatusLED] ?¤í????ìš© ?¤íŒ¨: %s", e)
    ...
    def _run_check(self) -> None:
        try:
            ...
            logger.debug("[DBStatusLED] %s ì²´í¬ ?ˆì™¸: %s", self.db_name, e)
            result = "red"
        finally:
            self._checking = False
    ...
    def _do_check(self) -> str:
        """
        health_check ëª¨ë“ˆ??ìºì‹œ?ì„œ ì°¾ê³ , ?†ë‹¤ë©??„ë³´ ?¤ìž„?¤íŽ˜?´ìŠ¤/?Œì¼ë¡???ë²ˆë§Œ ë¡œë“œ.
        ?´í›„?ëŠ” ìºì‹œ??ëª¨ë“ˆ???¨ìˆ˜ë§??¸ì¶œ?©ï¿½ï¿½ë‹¤.
        """
        ...
        if mod is None:
            logger.debug("[DBStatusLED] %s ì²´í¬ ëª¨ë“ˆ??ì°¾ì„ ???†ìŒ (key=%s)", self.db_name, key)
            return "red"
        ...
        try:
            ...
            logger.debug("[DBStatusLED] %s ì²´í¬ ?¤íŒ¨ during call: %s", self.db_name, e)
