# CHANGELOG
# 2026-03-16 | Copilot | 업그레이드: scanner README v4.0. 버전 통일.
# 2026-03-13 | Copilot | 업그레이드: scanner scanner/ → engine/ 변경.
# 2026-03-06 | Copilot | 생성: scanner README 초안
...
`scanner`는 Upbit 거래소 전체 종목을 실시간으로 스캔하여 사용자가 정의한 조건에 맞는 종목을 자동으로 찾아주는 모듈입니다.
...
```python
from src._scanner import ScannerEngine, ScannerWorker
...
```bash
pytest src/scanner/engine/tests/ -v
```
...
- `src/core/` : 설정 관리, 이벤트 버스 (스캔 결과 알림 발행)
- `src/data_01/timescale/` : OHLCV 캔들 데이터
- `src/data_01/redis/` : 스캔 결과 캐시
- `src/compute/` : 지표 계산 엔진 (O(1) 증분 계산)
