...
[Usage]
    from _core.database.redis_factory import get_redis_client, get_redis_url
...
def _load_redis_config() -> dict:
    ...
    try:
        ...
        # redis_factory.py is at src/core/database/redis_factory.py
        # config.yaml is at src/core/config/config.yaml
        config_path = Path(__file__).resolve().parent.parent / "config" / "config.yaml"
        if not config_path.exists():
            # Fallback: look for config.yaml from src/
            _src = Path(__file__).resolve().parent.parent.parent
            for candidate in [
                _src / "core" / "config" / "config.yaml",
