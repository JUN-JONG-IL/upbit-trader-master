...
def _discover_static_module():
    candidates = ("src.11_server.app.static", "11_server.app.static", "src.app.static", "app.static", "static")
...
def _instantiate_from_orchestrator(static_obj=None, cb=None):
    try:
        try:
            pkg = importlib.import_module("src.14_orchestrator")
        except Exception:
            try:
                pkg = importlib.import_module("14_orchestrator")
            except Exception:
                pkg = None
...
def _instantiate_from_file_fallback(cb=None):
    try:
        ...
        if mod is None:
            ...
            candidate = os.path.abspath(os.path.join(here, "..", "..", "14_orchestrator", "auto_backfill.py"))
