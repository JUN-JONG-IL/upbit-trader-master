...
PreLoginChecker QThread ë¥?ë¶„ë¦¬??ëª¨ë“ˆ.
- TCP ?¬íŠ¸ ?•ì¸ ë°?optional health_check ëª¨ë“ˆ ?¸ì¶œ
- UI ?¤ë ˆ?œë¡œ ?íƒœ dictë¥?emit ?©ë‹ˆ??
"""
...
class PreLoginChecker(QThread):
    ...
    def _try_health_module(self, key: str) -> Optional[bool]:
        """
        optional health ëª¨ë“ˆ??ì°¾ì•„ ?¤í–‰?©ë‹ˆ?? ëª¨ë“ˆ ?¨ìˆ˜ê°€ coroutine?????ˆìœ¼ë¯€ë¡?
        ?¸ì¶œ?ëŠ” ?„ìš”??ê²½ìš° ?ìœ„?ì„œ async-safe?˜ê²Œ ?¸ì¶œ?˜ë„ë¡?êµ¬í˜„?´ì•¼ ?©ë‹ˆ??
        ?¬ê¸°?œëŠ” ?¨ìˆœ ?™ê¸° ?¸ì¶œ(?¸ì¶œ?ì—???ˆì „???¼ë¡œ ?ì–´ ì±…ì„ ë¶„ë¦¬?©ë‹ˆ??
        """
        candidates = {
